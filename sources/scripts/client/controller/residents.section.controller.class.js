...
export class ResidentsSectionController extends Modal {
	...
	onLinkBarangay ( arg ){
		let args = arg.detail.query;
		this.barangay.id = args.BARANGAY_ID;
		this.barangay.name = args.BARANGAY_NAME;
		this.binds(this.controllerName,'#'+this.modalID);
		this.bindChildObject ( this , false );
		this.init();
		args.modal.onClose();
	...
	async viewClaimRecords () {
		let ssm = new DateRangeModal ({
			...
			params : {
				asyncRequest: this.mainService.viewClaimRecords,
				next: this.openClaimsReport,
				paramsMethod : {
					PUROK : this.purok,
	...
	openClaimsReport (result, context, params) {
		let _this = context;
		console.log(result)
		_this.mainService.openTab(
			"POST", 
			"/vims/sources/templates/reports/residents.benifit.claims.report2.php", {
				data : {
					res : result,
					param : params
				},
							...
							attributes : [
								{
									...
									value : ( selData ) => {
										//console.log(selData);
										if (selData['ITEM_IMAGE']=='') return '/vims/sources/images/prof.jpg';
									...
									value : ( argss ) => {
										...
										let dataQuery = {
											...
											data : {
												data : {
													...
													REQUEST_QUERY : [
														{
															sql : `select  RS.FULLNAME,RS.RESIDENT_ID,RS.BIRTHDAY,RS.RELATIONSHIP_TO_HH_LEADER from vims.barangay_res_setup RS
																	where RS.HH_LEADER = ?`,
															db : 'DB',
															index : 'hh_members',
															query_request : "GET",
														...
														{
															sql : `select * from barangays where BARANGAY_ID = ?`,
															db : 'DB',
															index : 'brgy',
															query_request : "GET",
															values : [argss.BARANGAY_ID]
														},
														{
										...
										this.mainService.serverRequest( dataQuery , ( res ) => {
											setTimeout ( ( ) =>{
												...
												if ( hhmems['brgy'][0] )
													argss['BARANGAY_NAME'] = hhmems['brgy'][0]['BARANGAY_NAME'];
	...
	init(){
		...
		if ( this.barangay.id !== "" )
			valsQuery.push ( this.barangay.id )
					...
					REQUEST_QUERY : [
						{
							sql : `SELECT * FROM vims.barangay_res_setup w WHERE CONCAT(LASTNAME,' ',FIRSTNAME,' ',MIDDLENAME) LIKE ? and w.ADDRESS like ?`+
								   ...
								   ( ( this.barangay.id !== "" ) ? ` and w.BARANGAY_ID = ?` : "") +
								   ` ORDER BY w.ID ASC `
								   ,
							db : 'DB',
							query_request : 'GET',
